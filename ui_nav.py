...
PAGES = {
    ...
    # se você ainda não tiver esses arquivos, pode comentar:
...
def render_topbar_nav(active: str):
    """
    Topbar fina e discreta.
    Navegação via ?go=... + switch_page.
    """
    _nav_if_requested()
...
.omni-dot{{width: 20px; height: 2px; border-radius: 999px; margin-top:2px; background: rgba(15,23,42,0.18);}}
...
""",
    ...
    logo_html = (
        f"<img class='omni-mark' src='data:image/png;base64,{logo_b64}'/>"
        if logo_b64
        else "<div class='omni-mark' style='background:conic-gradient(from 0deg,#3B82F6,#22C55E,#F59E0B,#F97316,#A855F7,#3B82F6);'></div>"
    )
