...
default_state = {
    ...
    'status_validacao_pei': 'rascunho', # rascunho, aprovado, ajustando
    'feedback_ajuste': ''
}
...
def extrair_bloom(texto):
    ...
    # Retorna apenas os verbos para a nuvem de tags, remove explicações longas se houver
    return [v.split(':')[0].strip() for v in bloco.split(',')]
...
def limpar_texto_pdf(texto):
    """Limpa e garante encoding latin-1 para evitar erros no FPDF"""
    if not texto: return ""
    # Substituições comuns de markdown/erros
    t = texto.replace('**', '').replace('__', '').replace('#', '').replace('•', '-')
    # Tenta codificar e decodificar para garantir compatibilidade
...
# ==============================================================================
# FUNÇÃO DE PROGRESSO AJUSTADA (LOGO GIRANDO)
# ==============================================================================
...
def extrair_dados_pdf_ia(api_key, texto_pdf):
    ...
    try:
        ...
        {{ "diagnostico": "Texto...", "medicamentos": [ {{"nome": "...", "posologia": "..."}} ] }}
...
def consultar_gpt_pedagogico(api_key, dados, contexto_pdf="", modo_pratico=False, feedback_usuario=""):
    ...
    try:
        ...
        Escreva um parágrafo humanizado. Mostre quem é a criança por trás do diagnóstico.
        [/PERFIL_NARRATIVO]
        """
        ...
        if "Alfabético" not in alfabetizacao and alfabetizacao != "Não se aplica (Educação Infantil)":
             ...
             Fase atual: {alfabetizacao}.
             OBRIGATÓRIO: Inclua 2 ações específicas de consciência fonológica para avançar a hipótese de escrita.
             [/ATENÇÃO CRÍTICA]
             """

        if nivel_ensino == "EI":
            # === EDUCAÇÃO INFANTIL (BNCC) ===
            perfil_ia = "Você é um Especialista em EDUCAÇÃO INFANTIL e Inclusão. FOCO: BNCC (Campos de Experiência)."
            estrutura_req = f"""
            ESTRUTURA OBRIGATÓRIA (EI):
            {prompt_identidade}
            1. 🌟 AVALIAÇÃO DE REPERTÓRIO:
            [ANALISE_FARMA] Análise farmacológica e impacto. [/ANALISE_FARMA]
            [CAMPOS_EXPERIENCIA_PRIORITARIOS]
            Destaque 2 ou 3 Campos de Experiência BNCC. Use emojis.
            [/CAMPOS_EXPERIENCIA_PRIORITARIOS]
            ...
            (Adaptações sensoriais e rotina).
            """
        else:
            # === FUNDAMENTAL E MÉDIO (BLOOM, SMART, BNCC CÓDIGOS) ===
            perfil_ia = "Você é um Especialista em Inclusão Escolar e BNCC."
            
            # Instrução Específica para Mapeamento BNCC
            instrucao_bncc = """
            [MAPEAMENTO_BNCC]
            Separe as habilidades por ÁREA DO CONHECIMENTO ou COMPONENTE CURRICULAR (ex: Matemática, Linguagens).
            Para cada habilidade, VOCÊ DEVE INCLUIR O CÓDIGO ALFANUMÉRICO DA BNCC (ex: EF01LP02).
            - **Habilidades Basais (Pré-requisitos):** O que precisa ser resgatado de anos anteriores (com códigos BNCC).
            - **Habilidades Focais (Ano Atual):** O que é essencial para este ano (com códigos BNCC).
            [/MAPEAMENTO_BNCC]
            """

            # Instrução Específica para Bloom
            instrucao_bloom = """
            [TAXONOMIA_BLOOM] 
            Explique brevemente a categoria cognitiva da Taxonomia Revisada (ex: Recordar, Entender, Criar) escolhida para o aluno.
            Em seguida, liste 3 verbos de comando adequados.
            [/TAXONOMIA_BLOOM]
            """
            ...
            {prompt_identidade}
            1. 🌟 AVALIAÇÃO DE REPERTÓRIO:
            [ANALISE_FARMA] Análise farmacológica. [/ANALISE_FARMA]
        ...
        if feedback_usuario:
            prompt_feedback = f"""
            ATENÇÃO: O professor solicitou o seguinte ajuste no plano anterior:
            "{feedback_usuario}"
            REFAÇA O PLANO CONSIDERANDO ESTA SOLICITAÇÃO PRIORITARIAMENTE.
            """
        ...
        if modo_pratico:
            prompt_sys = f"""
            {perfil_ia}
            SUA MISSÃO: Criar um GUIA PRÁTICO E DIRETO para o professor usar em sala de aula.
            {prompt_feedback}
            ESTRUTURA:
            # GUIA PRÁTICO PARA {serie.upper()}
            {prompt_identidade}
            1. 🎯 O QUE FAZER AMANHÃ: (3 ações simples).
            2. 🗣️ COMO FALAR: (Comandos e feedbacks).
        ...
        DIAGNÓSTICO: {dados['diagnostico']} | ALFABETIZAÇÃO: {alfabetizacao}
        MEDICAÇÃO: {meds_info} | HIPERFOCO: {dados['hiperfoco']}
...
def gerar_roteiro_gamificado(api_key, dados, pei_tecnico):
    ...
    try:
        ...
        if nivel_ensino == "EI":
            prompt_sys = "Crie uma História Visual (4-5 anos) com emojis. Estrutura: # ☀️ AVENTURA ... Chegada, Atividades, Lanche, Descanso, Saída."
        elif nivel_ensino == "FI":
            prompt_sys = "Crie um Quadro de Missões (6-10 anos) RPG. Estrutura: # 🗺️ MAPA ... Equipamento, Super Poder, O Desafio, Recompensa, Aliados."
        else:
            prompt_sys = "Crie uma Ficha de Personagem RPG (Adolescente). Estrutura: # ⚔️ FICHA ... Quest, Skills, Buffs, Boss, Mana."
        
        res = client.chat.completions.create(model="gpt-4o-mini", messages=[{"role": "system", "content": prompt_sys}, {"role": "user", "content": contexto_seguro}])
        return res.choices[0].message.content, None
    except Exception as e: return None, str(e)
...
class PDF_Classic(FPDF):
    ...
    def add_flat_icon_item(self, texto, bullet_type='check'):
        ...
        self.set_text_color(80, 80, 80)
        char = '3'
        if bullet_type == 'arrow': char = 'PARAGRAPH'
        elif bullet_type == 'dot': char = 'l'
...
def gerar_pdf_final(dados, tem_anexo):
    ...
    if dados['lista_medicamentos']:
        ...
        for m in dados['lista_medicamentos']:
            ...
            pdf.add_flat_icon_item(limpar_texto_pdf(txt), 'dot')

    evidencias = [k.replace('?', '') for k, v in dados['checklist_evidencias'].items() if v]
    if evidencias:
        pdf.section_title("Evidências Observadas")
        for ev in evidencias:
            # CORREÇÃO CIRÚRGICA: Limpeza rigorosa para evitar erro no PDF
            pdf.add_flat_icon_item(limpar_texto_pdf(ev), 'arrow')
    ...
    if dados['ia_sugestao']:
        ...
        # Remove tags internas (mas mantém o texto)
...
with tab0:
    ...
    <div class="soft-card sc-blue" style="min-height: auto; margin-bottom: 20px;">
        ...
        <div class="sc-body">O <b>Plano de Ensino Individualizado (PEI)</b> é o instrumento norteador da inclusão escolar. Ele baseia-se no Desenho Universal para a Aprendizagem (DUA) e deve ser revisado periodicamente.</div>
    ...
    <div class="soft-card sc-yellow" style="min-height: auto;">
        ...
        <div class="sc-body">A fundamentação legal baseia-se na <b>LDB</b> e na LBI.<br><br><b>📢 ATUALIZAÇÕES:</b> Decretos 12.686 e 12.773 (Dez/2025).</div>
...
with tab1:
    ...
    with st.container(border=True):
        ...
        if usa_med:
            ...
            admin_escola = c3.checkbox("Na escola?", key="adm_esc")
            if st.button("Adicionar"):
                st.session_state.dados['lista_medicamentos'].append({"nome": nm, "posologia": pos, "escola": admin_escola}); st.rerun()
        if st.session_state.dados['lista_medicamentos']:
            st.write("---")
            for i, m in enumerate(st.session_state.dados['lista_medicamentos']):
                tag = " [NA ESCOLA]" if m.get('escola') else ""
                c_txt, c_btn = st.columns([5, 1]); c_txt.info(f"💊 **{m['nome']}** ({m['posologia']}){tag}")
...
with tab6:
    ...
    st.session_state.dados['monitoramento_data'] = st.date_input("Data da Próxima Revisão", value=st.session_state.dados.get('monitoramento_data', None))
    st.divider()
...
with tab7: # IA (CONSULTORIA PEDAGÓGICA) - FLUXO DE VALIDAÇÃO
    render_progresso()
    ...
    # Se não tem sugestão ou está 'resetado', mostra botão de gerar
    if not st.session_state.dados['ia_sugestao'] or st.session_state.dados.get('status_validacao_pei') == 'rascunho':
        ...
        with col_info:
            st.info("Clique para gerar o plano inicial. Você poderá revisar e solicitar ajustes em seguida.")

    # MODO REVISÃO E APROVAÇÃO
    elif st.session_state.dados.get('status_validacao_pei') in ['revisao', 'aprovado']:
        
        # Exibição do Texto Renderizado (Sem tags feias)
        st.markdown("#### 📝 Revisão do Plano")
        
        # Limpa tags técnicas da visualização em tela, mas mantém para o PDF
        texto_visual = re.sub(r'\[.*?\]', '', st.session_state.dados['ia_sugestao'])
        st.markdown(texto_visual)
        
        st.divider()
        st.markdown("**⚠️ Responsabilidade do Educador:** A IA pode cometer erros. Este documento deve ser editado e validado por você.")
        ...
        if st.session_state.dados.get('status_validacao_pei') == 'revisao':
            ...
            if c_ok.button("✅ Aprovar Plano", type="primary", use_container_width=True):
                ...
                st.success("Plano aprovado! Você pode editar manualmente abaixo se precisar de detalhes finais.")
                st.rerun()
    ...
    # MODO AJUSTE (LOOP)
        ...
        st.warning("Descreva o que precisa ser alterado no plano:")
        feedback = st.text_area("Seu feedback para a IA:", placeholder="Ex: Aumente o nível de dificuldade em matemática, foque mais na alfabetização...")
    ...
    if st.session_state.dados['nome']:
        ...
        with c_r2:
            ...
            if nivel == "EI":
                ...
                card_title = "Campos de Experiência (BNCC)"; card_icon = "🧸"
            else:
                itens = extrair_bloom(st.session_state.dados['ia_sugestao'])
                card_title = "Taxonomia de Bloom (Verbos)"; card_icon = "🧠"
        ...
        if st.session_state.dados['ia_sugestao']:
            ...
            with c2:
                ...
                st.write(""); st.write(""); st.markdown("**📂 Backup de Dados (.json)**")
                st.caption("ℹ️ Salve na pasta 'Meus Alunos PEI'.")
                st.download_button("💾 Salvar Arquivo", json.dumps(st.session_state.dados, default=str), f"PEI_{st.session_state.dados['nome']}.json", "application/json")
        st.markdown("---")
        if st.button("🚀 SALVAR E SINCRONIZAR (PAE/HUB)", type="primary"):
            ok, msg = salvar_aluno_integrado(st.session_state.dados)
...
with tab_mapa:
    ...
    if st.session_state.dados['ia_sugestao']:
        ...
        if st.session_state.dados['ia_mapa_texto']:
            st.markdown("### 📜 Roteiro"); st.markdown(st.session_state.dados['ia_mapa_texto']); st.divider()
            pdf_mapa = gerar_pdf_tabuleiro_simples(st.session_state.dados['ia_mapa_texto'])
            st.download_button("📥 Baixar Missão PDF", pdf_mapa, f"Missao_{st.session_state.dados['nome']}.pdf", "application/pdf", type="primary")
            if st.button("Recomeçar Mapa"): st.session_state.dados['ia_mapa_texto'] = ""; st.rerun()
