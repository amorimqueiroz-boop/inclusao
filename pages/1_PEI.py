...
def get_logo_omnisfera():
    # Logo que gira (Badge) - Prioriza Omnisfera
...
def get_logo_pei():
    # Logo do cabeçalho - Prioriza PEI 360
...
class PDF_Classic(FPDF):
    def header(self):
        ...
        img = get_base64_image(finding_logo_pei()) # Logo PEI no topo
        if img: 
...
# ==============================================================================
with st.sidebar:
    img_html = f'<img src="data:image/png;base64,{get_base64_image(finding_logo_pei())}" style="width: 120px;">'
    if finding_logo_pei(): st.markdown(img_html, unsafe_allow_html=True)
        ...
        if st.button("📥 Baixar da Nuvem"):
            with st.spinner("Buscando dados..."):
                ...
                if dados_recuperados:
                    ...
                    # BLINDAGEM CONTRA KEYERROR AO CARREGAR
                    if 'barreiras_selecionadas' not in dados_recuperados: 
                        dados_recuperados['barreiras_selecionadas'] = {}
                    for k in LISTAS_BARREIRAS.keys():
                        if k not in dados_recuperados['barreiras_selecionadas']: 
