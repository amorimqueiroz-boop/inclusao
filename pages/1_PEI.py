...
def render_progresso():
    ...
    # SUBSTITUIÇÃO CIRÚRGICA: Emoji por Imagem da Logo Girando
...
with tab1: # ESTUDANTE
    ...
    # --- CORREÇÃO CIRÚRGICA DE ERRO DE LISTA ---
    # Filtra os defaults para garantir que existam na lista de opções, evitando StreamlitAPIException
...
with tab6: # MONITORAMENTO - ALTERAÇÃO CIRÚRGICA (SEPARAÇÃO DE CAMPOS)
    render_progresso()
    ...
    # Aviso de separação para o momento da revisão
    st.warning("⚠️ **ATENÇÃO:** Os campos abaixo devem ser preenchidos **somente no momento da revisão** do PEI.")
    
    with st.container(border=True):
        st.markdown("**Resultado da Revisão**")
        c2, c3 = st.columns(2)
        with c2: 
            st.session_state.dados['status_meta'] = st.selectbox("Status da Meta Atual", ["Não Iniciado", "Em Andamento", "Parcialmente Atingido", "Atingido", "Superado"], index=0)
        with c3: 
            st.session_state.dados['parecer_geral'] = st.selectbox("Parecer Geral", ["Manter Estratégias", "Aumentar Suporte", "Reduzir Suporte (Autonomia)", "Alterar Metodologia", "Encaminhar para Especialista"], index=0)
...
with tab8: # DASHBOARD & DOCS (RENOMEADO)
    ...
    if st.session_state.dados['nome']:
        ...
        # ALTERAÇÃO CIRÚRGICA: CARD NÍVEL DE ATENÇÃO SIMPLIFICADO
        with c_kpi4:
        ...
        if st.session_state.dados['ia_sugestao']:
            ...
            with c2:
                ...
                # ALTERAÇÃO CIRÚRGICA: EXPLICAÇÃO DO JSON
                st.write("")
                st.write("")
                st.markdown("**📂 Backup de Dados (.json)**")
                st.caption("ℹ️ **Instrução:** Crie uma pasta no seu computador chamada 'Meus Alunos PEI'. Salve este arquivo lá para poder recarregar e editar os dados futuramente.")
                
                json_dados = json.dumps(st.session_state.dados, default=str)
                st.download_button("💾 Salvar Arquivo do Aluno", json_dados, f"PEI_{st.session_state.dados['nome']}.json", "application/json")
                st.write("")
