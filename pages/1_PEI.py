...
def verificar_acesso():
    if "autenticado" not in st.session_state or not st.session_state["autenticado"]:
        # Se não houver login, assume logado para teste ou exibe erro
        # st.error("🔒 Acesso Negado. Por favor, faça login na Página Inicial.")
        # st.stop()
        pass # Bypass temporário se necessário, ou descomentar acima
# verificar_acesso() # Comentado para facilitar testes se não houver login prévio

# ==============================================================================
# 2. LÓGICA DO BANCO DE DADOS (GOOGLE SHEETS INTEGRATION)
# ==============================================================================
# Conecta ao Google Sheets usando as credenciais do st.secrets
@st.cache_resource
def conectar_gsheets():
    scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
    # Carrega credenciais do secrets.toml [gcp_service_account]
...
def carregar_banco():
    ...
    try:
        ...
        # Tenta abrir a planilha. Se não existir, teria que criar (processo manual recomendado primeiro)
        # Substitua "Omnisfera_Dados" pelo nome exato da sua planilha no Google Drive
        sheet = client.open("Omnisfera_Dados").sheet1 
        records = sheet.get_all_records()
        
        # Processa os registros para converter o JSON string de volta para dict
        lista_processada = []
        for reg in records:
            try:
                ...
                if 'Dados_Completos' in reg and reg['Dados_Completos']:
                    ...
                    # Fallback para dados simples da planilha
...
def salvar_aluno_integrado(dados):
    ...
    try:
        ...
        # Prepara o JSON completo para salvar na nuvem (permite recuperar tudo depois)
        dados_json_str = json.dumps(dados, default=str, ensure_ascii=False)
        
        # Dados das colunas principais (para leitura humana na planilha)
        linha_dados = [
            dados['nome'],
            dados.get('serie', ''),
            ...
            dados_json_str # Coluna F: O "Cérebro" do dado
        ]
        
        # Lógica: Atualizar se existir, Adicionar se novo
        cell = None
        try:
        ...
        if cell:
            # Atualiza a linha existente (presumindo estrutura fixa)
            # Nota: Isso atualiza a linha inteira baseada na ordem das colunas
            # Ajuste o range conforme suas colunas. Aqui assume A a F.
        ...
        st.session_state.banco_estudantes.append(dados)
...
st.markdown("<div class='footer-signature'>PEI 360º v120.0 (Google Sheets Edition) - Desenvolvido por Rodrigo A. Queiroz</div>", unsafe_allow_html=True)
