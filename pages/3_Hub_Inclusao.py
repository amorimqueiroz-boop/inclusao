...
# ==============================================================================
LÓGICA DO BANCO DE DADOS (GOOGLE SHEETS + LOCAL)
# ==============================================================================
import json
...
def carregar_banco():
    ...
    try:
        ...
        # Tenta abrir a primeira aba para carregar alunos
...
# === NOVA FUNÇÃO CIRÚRGICA PARA O RASTRO ===
def salvar_rastro(nome_aluno, tipo_atividade, contexto, detalhe):
    """
    Salva o log de validação na aba 'Rastro' da planilha Omnisfera_Dados.
    ...
    try:
        ...
        # Tenta pegar a aba 'Rastro', se der erro, cria ela
        try:
            sheet_rastro = planilha.worksheet("Rastro")
        except:
            sheet_rastro = planilha.add_worksheet(title="Rastro", rows=1000, cols=6)
            # Adiciona cabeçalho se acabou de criar
            sheet_rastro.append_row(["Data Hora", "Aluno", "Atividade", "Contexto", "Detalhe", "Responsável"])
            
        # Dados para salvar
        data_hora = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        usuario = st.session_state.get("usuario_nome", "Admin") # Pega usuário logado ou põe Admin
        
        # Garante que detalhe seja string para não quebrar o gspread
        linha = [data_hora, nome_aluno, tipo_atividade, contexto, str(detalhe), usuario]
        
        sheet_rastro.append_row(linha)
        return True
    except Exception as e:
        # Imprime erro no terminal do Streamlit para debug silencioso, mas retorna False
