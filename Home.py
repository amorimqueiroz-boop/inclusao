...
<style>
    ...
    /* --- 3. HEADER DE VIDRO (ALINHADO À ESQUERDA) --- */
    .logo-container {{
        display: flex; 
        align-items: center; 
        justify-content: flex-start; /* ALINHAMENTO À ESQUERDA */
        gap: 30px; 
        padding: 0 50px; /* Mais padding lateral */
    ...
    .logo-icon-spin {{ height: 55px; width: auto; animation: spin 45s linear infinite; filter: drop-shadow(0 4px 8px rgba(0,0,0,0.1)); }}
    
    /* NOME OMNISFERA (EMBAIXO DA LOGO) */
    .logo-text-static {{ height: 22px; width: auto; margin-top: 4px; }}

    /* TEXTO DIREITA (ECOSSISTEMA) - COM BARRA SEPARADORA */
    .header-slogan {{
        font-family: 'Nunito', sans-serif;
        font-weight: 700;
        font-size: 1.1rem;
        ...
        text-align: left; 
        line-height: 1.2;
        border-left: 2px solid #E2E8F0; /* LINHA DIVISÓRIA */
        padding-left: 30px; 
        height: 50px; 
        display: flex;
        align-items: center;
    ...
    .omni-badge {{
        position: fixed; top: 120px; right: 20px; /* Baixo do header */
        background: {card_bg};
...
def sistema_seguranca():
    ...
    st.markdown(f"""
        <style>
            ...
            /* Ocultar aviso de teste amarelo */
            .teste-warning {{ display: none !important; }} 
        </style>
    """, unsafe_allow_html=True)
    ...
    if not st.session_state["autenticado"]:
        ...
        with c_login:
            ...
            if IS_TEST_ENV:
                # MODO TESTE (Sem retângulo amarelo)
